...
interface CartState {
  ...
  // Proves this browser is the one that identified as customerId -- sent
  // on every interaction call so a third party who merely observes/guesses
  // a customerId can't spam interactions onto someone else's ticket.
  ...
  persist(
    (set, get) => ({
      ...
      logEvent: (type, detail) => {
        const { customerId, sessionToken } = get();
        if (customerId && sessionToken) {
          api.logInteraction(customerId, sessionToken, type, detail).catch(() => {});
        } else {
      ...
      identify: async (email, name) => {
        ...
        const { customerId, sessionToken } = await api.identifyCustomer({
          email,
          name,
          cartId: cartId ?? undefined,
          bufferedEvents: pendingEvents,
        });
        set({ customerId, customerEmail: email, sessionToken, pendingEvents: [], emailPromptDismissed: true });
      },
    ...
    {
      name: "oneix-cart",
      // Next.js renders this store once on the server (no localStorage
      // there) and once on the client -- auto-rehydrating on the client
      // would diverge from the server-rendered markup and trigger a
      // hydration mismatch. Skip auto-rehydration; ClientProviders calls
      // useCartStore.persist.rehydrate() itself after mount instead.
      skipHydration: true,
      partialize: (state) => ({
        cartId: state.cartId,
